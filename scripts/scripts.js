...
/**
 * Applies Section Metadata to each section as CSS classes, then removes the
 * metadata block. The vendored aem.js decorateSections() omits this standard
 * EDS step, so a `Section Metadata` table (e.g. Style: grey) would otherwise
 * render as visible content and 404 as an unknown block. Run before
 * decorateSections so the classes land on the section wrapper.
...
function applySectionMetadata(main) {
  main.querySelectorAll(':scope > div > div.section-metadata').forEach((meta) => {
    ...
    Object.keys(config).forEach((key) => {
      if (key === 'style') {
        const styles = config.style.split(',').map((s) => toClassName(s.trim())).filter(Boolean);
